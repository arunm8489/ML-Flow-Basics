...
import sys
import warnings
from utils.sagemaker_integration import *
...
if __name__ == "__main__":
    ...
    for i,(no_estimators,max_depth) in enumerate([(100,5),(100,7),(200,5)]):
        run_name = f'RF_{str(i)}'
        train_model(no_estimators,max_depth,run_name)

    
    try:
        if input('Push models to s3? [Y or N] ') == 'Y':
      
            upload_folder = "artifacts"
            
            print(f'Path exists: {os.path.exists(upload_folder)}')
            bucket_name = 'mlflow-artifacts001'
            status = upload(bucket_name,upload_folder)
            print(status)
    
    except:
        print('Error in upoading')
